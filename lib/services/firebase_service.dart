...
class FirebaseService {
  ...
  static Future<bool> initializeAuth() async {
    try {
      ...
      if (configured) {
        ...
        print(
          'Firebase Auth configuration failed, but continuing with fallback auth',
        );
      }
      return configured;
    } catch (e) {
  ...
  Future<void> logPasswordResetAttempt({
    ...
    try {
      ...
      print(
        'Password reset attempt logged: $email, method: $method, success: $success',
      );
    } catch (e) {
  ...
  Future<UserCredential?> signInWithEmailAndPassword(
    ...
    try {
      ...
      if (!configured) {
        print(
          'Firebase Auth configuration failed, skipping Firebase Auth sign in',
        );
        return null;
      }
      ...
      if (e is FirebaseAuthException) {
        if (e.message?.contains('CONFIGURATION_NOT_FOUND') == true ||
            ...
            e.code == 'timeout') {
          print(
            'Firebase Auth failed with ${e.code}, will try direct Firestore login',
          );
          return null;
        }
  ...
  Future<Map<String, dynamic>?> authenticateWithFirestore(
    String email,
    String password,
  ) async {
    try {
      ...
      final QuerySnapshot userQuery = await _firestore
          ...
          .where(
            'password',
            isEqualTo: password,
          ) // We stored the password in Firestore for this fallback
          .limit(1)
          .get();
  ...
  Future<UserCredential?> registerWithEmailAndPassword(
    ...
    try {
      ...
      if (!configured) {
        print(
          'Firebase Auth configuration failed, skipping Firebase Auth registration',
        );
        return null;
      }
      ...
      if (e is FirebaseAuthException) {
        ...
        if (e.message?.contains('CONFIGURATION_NOT_FOUND') == true ||
            ...
            e.code == 'timeout') {
          print(
            'Firebase Auth failed with ${e.code} issue, will try direct Firestore registration',
          );
          return null;
        }
      }
  ...
  Future<Map<String, dynamic>> registerDirectlyToFirestore(
    Map<String, dynamic> userData,
    UserRole role,
  ) async {
    try {
      ...
      userData['isDirectRegistration'] =
          true; // Flag to identify direct registrations
      ...
      try {
        ...
        print(
          'User data stored in ${role.toString().split('.').last}s collection',
        );
  ...
  Future<void> updateUserData(String uid, Map<String, dynamic> data) async {
    try {
      ...
      if (role != null) {
        String collection =
            role.toString().split('.').last + 's'; // admin -> admins
