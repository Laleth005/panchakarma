...
class PractitionerRepository {
  ...
  Future<List<TherapySessionModel>> getTodaySessions(String practitionerId) async {
    try {
  ...
  Future<bool> updateSessionStatus(String sessionId, SessionStatus newStatus) async {
    try {
      await _firestore
          .collection('therapy_sessions')
          .doc(sessionId)
  ...
  Future<List<PatientFeedbackModel>> getPendingFeedback(String practitionerId) async {
    try {
      final snapshot = await _firestore
  ...
  Future<List<TaskReminderModel>> getTaskReminders(String practitionerId) async {
    try {
      final snapshot = await _firestore
  ...
  Future<bool> addTaskReminder(TaskReminderModel task, String practitionerId) async {
    try {
  ...
  // Get analytics data
  Future<Map<String, dynamic>> getPractitionerAnalytics(String practitionerId) async {
    final now = DateTime.now();
    ...
    try {
      ...
      final todaySessionsSnapshot = await _firestore
          ...
          .where('status', isEqualTo: SessionStatus.completed.toString().split('.').last)
          .get();
