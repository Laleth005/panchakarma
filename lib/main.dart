    ...
    for (int attempt = 1; attempt <= 3 && !authConfigured; attempt++) {
      try {
        ...
        if (authConfigured) {
          ...
          print(
            'Firebase Auth configuration attempt $attempt failed, will retry',
          );
        }
      } catch (authError) {
        print(
          'Error during Firebase Auth configuration attempt $attempt: $authError',
        );
    ...
    return MaterialApp(
      ...
      routes: {
        ...
        '/practitioner/dashboard': (context) =>
            const PractitionerMainDashboard(practitionerId: ''),
